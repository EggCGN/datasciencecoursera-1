...
gdp$Gross.domestic.product.2012 = sapply(sapply(gdp$Gross.domestic.product.2012, as.character), as.numeric)
gdp = gdp[complete.cases(gdp[,"Gross.domestic.product.2012"]),]
head(gdp)
head(edu)
result = merge(edu, gdp, by = "CountryCode") %>% arrange(desc(Gross.domestic.product.2012))
result[13,]
nrow(result)
```
...
```{r}
result[result$Income.Group %in% c("High income: OECD","High income: nonOECD"),]
```
